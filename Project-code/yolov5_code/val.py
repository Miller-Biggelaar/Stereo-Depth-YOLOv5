...
import pandas as pd  # for saving evaluation metrics to CSV
...
def run(
    ...
    results_dict = {
        ...
        "mAP@0.5:0.95": [map],
        "Images": [seen],
        "Instances": [int(nt.sum())]
    }
    results_csv = Path(save_dir) / "results_summary.csv"
    pd.DataFrame(results_dict).to_csv(results_csv, index=False)
    LOGGER.info(f"✅ Saved evaluation metrics to {results_csv}")
