...
def compute_disparity(rect_left, rect_right):
    ...
    block_size          = 9     # TRY 11-13 - smaller block size = more local detail and more sensitive to noise
    ...
    stereo = cv2.StereoSGBM_create(
        ...
        mode=cv2.STEREO_SGBM_MODE_SGBM_3WAY #TRY 
    )
...
def process_existing_images():
    ...
    for left_file in left_files:
        # Derive matching right filename
        # Replace 'left_' with 'right_' to find the corresponding file
        right_file = left_file.replace('left_', 'right_')

        if right_file not in right_files:
            print(f"[!] No matching right image for {left_file}, skipping.")
            continue

        left_path = os.path.join(left_dir, left_file)
        right_path = os.path.join(right_dir, right_file)

        print(f"\n[+] Processing pair: {left_file} | {right_file}")
        ...
        if img_left is None or img_right is None:
            print(f"[!] Could not read one of the images: {left_path} or {right_path}")
            continue
        ...
        # Save output files
        base_name = os.path.splitext(left_file)[0].replace('left_', '')
        cv2.imwrite(os.path.join(SAVE_DIR, f'disparity_{base_name}.jpg'), disp_vis)
        cv2.imwrite(os.path.join(SAVE_DIR, f'depth_{base_name}.jpg'), depth_vis)
        np.save(os.path.join(SAVE_DIR, f'depth_{base_name}.npy'), depth_map)

        print(f"    Saved outputs for {base_name} to {SAVE_DIR}")
